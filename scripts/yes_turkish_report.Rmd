...
+ Hopelessness decreased, agency increased, and there was no evidence self-hate changed from pre to post SSI (dzs: `r all_dz_values`)
+ We calculated changes pre to post SSI (vs by SSI) given limited sample sizes (e.g., df = `r d.dep.t.diff.t(abc_hate_t_value, abc_hate_n_pairs, a = 0.05)$df`)
...
```
+ **self-hate decreased** from pre to post SSI, dz = `r all_hate_dz_value`.

```{r t test results all hate, echo=FALSE}
