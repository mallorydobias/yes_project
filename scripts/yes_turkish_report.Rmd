...
```{r running t tests across all ssis using custom function, include=FALSE}

# BHS
results_bhs <- run_paired_t_test(data, "pre_bhs_mean", "post_bhs_mean", "all_bhs")
print(results_bhs)

# SHS
results_shs <- run_paired_t_test(data, "pre_shs_mean", "post_shs_mean", "all_shs")
print(results_shs)

# Self-hate
results_hate <- run_paired_t_test(data, "pre_self_hate_mean", "post_self_hate_mean", "all_hate")
print(results_hate)

# Combine all dz values into a single vector for reporting
all_dz_values <- c(all_bhs_dz_value, all_shs_dz_value, all_hate_dz_value) # not including non-significant outcome results
print(all_dz_values)
...
+ Hopelessness decreased, agency increased, and self-hate did not change from pre to post SSI (dzs: `r all_dz_values`)
+ We calculated pre to post changes overall (vs by SSI), given limited sample sizes (e.g., dfs ~ `r d.dep.t.diff.t(abc_hate_t_value, abc_hate_n_pairs, a = 0.05)$df`)
+ We plotted individual outcome trajectories within each SSI, for visibility
...
**People agreed to questionnaires almost every time (`r perc_agreed_questionnaires`%, n = `r n_agreed_questionnaires`)** they were asked.
...
Age(s), of all survey starts (n = `r n_started_survey`). 
...
Sex of all survey starts (n = `r n_started_survey`): 
...
**Nearly a third of those who started the survey (`r perc_elev_dep`%) reported elevated levels of depression symptoms.** Nearly *half* of all cases (`r perc_elev_dep_of_answered_dep`%) that actually responded to the depression items reported elevated levels of depression symptoms. Here, we plot depression scale scores (MFQ) across the entire sample.
...
Given small sample sizes within each SSI group, we analyze changes in scores from pre to post SSI (across all SSIs) for hopelessness, agency, and self-hate. Our paired t-tests find:

+ **hopelessness decreased** from pre to post SSI, dz = `r all_bhs_dz_value`.

```{r t test results all bhs, echo=FALSE}

# t_all_bhs # full results here

# Pulling table values

data_t_all_bhs <- broom::tidy(t_all_bhs) %>%
  select(statistic, parameter, p.value) %>%
  rename(df = parameter) %>%
  mutate(t = round(statistic, 2)) %>%
  mutate(dz = all_bhs_dz_value,
         all_test = "Hopelessness") %>%
  select(all_test, t, df, p.value, dz)

```

+ **agency increased** from pre to post SSI, dz = `r all_shs_dz_value`.

```{r t test results all shs, echo=FALSE}

# t_all_shs # full results here

# Pulling table values

data_t_all_shs <- broom::tidy(t_all_shs) %>%
  select(statistic, parameter, p.value) %>%
  rename(df = parameter) %>%
  mutate(t = round(statistic, 2)) %>%
  mutate(dz = all_shs_dz_value,
         all_test = "Agency") %>%
  select(all_test, t, df, p.value, dz)

```

+ **no evidence self-hate decreased** from pre to post SSI, dz = `r all_hate_dz_value`.

```{r t test results all hate, include=FALSE}

t_all_hate # full results here

# Pulling table values

data_t_all_hate <- broom::tidy(t_all_hate) %>%
  select(statistic, parameter, p.value) %>%
  rename(df = parameter) %>%
  mutate(t = round(statistic, 2)) %>%
  mutate(dz = all_hate_dz_value,
         all_test = "Self-Hate") %>%
  select(all_test, t, df, p.value, dz)

```

Paired t-test results for all SSIs:

```{r all t results, echo=FALSE}

# Combining t results

t_results_all <- bind_rows(data_t_all_bhs, data_t_all_shs, data_t_all_hate)

# Now, use nice_table for all abc results

rempsyc::nice_table(t_results_all)
...
For those who chose the **ABC Project** with data at both time points, we plot individual outcome trajectories.
...
For those who chose **Project CARE** with data at both time points, we plot individual outcome trajectories.

```{r running custom plot function on care bhs data, include=FALSE}

## Running plot setup function

plot_data_bhs_care_function <- process_plot_data(data, "CARE", "pre_bhs_mean", "post_bhs_mean", "hopelessness")
...
plot_care_bhs <- plot_data_bhs_care_function %>%
  ...
  mutate(line_color = case_when(
    hopelessness[timing == "After"] > hopelessness[timing == "Before"] ~ "grey",  # If After > Before, set color to grey
    hopelessness[timing == "After"] < hopelessness[timing == "Before"] ~ "#A52B4D",  # If After < Before, set color
    TRUE ~ "grey"  # Default case (in case of ties or missing values)
  )) %>%
  ungroup() %>%
  ggplot(aes(x = timing, y = hopelessness, group = response_id, color = line_color)) +  # Use line_color for coloring lines
  geom_line(alpha = .7, size = 1.1) +  # Increase size to make the lines thicker (adjust the value as needed)
  geom_point(size = 2) +  # Add points to highlight individual data points
  scale_color_manual(values = c("#A52B4D" = "#A52B4D", "grey" = "grey")) +  # Set color for decreased and increased scores
  theme_light() +
  theme(legend.position = "none",  # Remove legend, as it's not needed for this plot
        text = element_text(family = "noto sans"),
        title = element_text(size = 10),
...
plot_data_shs_care_function <- process_plot_data(data, "CARE", "pre_shs_mean", "post_shs_mean", "hope")
...
plot_care_shs <- plot_data_shs_care_function %>%
  ...
  mutate(line_color = case_when(
    hope[timing == "After"] < hope[timing == "Before"] ~ "grey",  # If After > Before, set color to grey
    hope[timing == "After"] > hope[timing == "Before"] ~ "#A52B4D",  # If After < Before, set color
    TRUE ~ "grey"  # Default case (in case of ties or missing values)
  )) %>%
  ungroup() %>%
  ggplot(aes(x = timing, y = hope, group = response_id, color = line_color)) +  # Use line_color for coloring lines
...
plot_data_hate_care_function <- process_plot_data(data, "CARE", "pre_self_hate_mean", "post_self_hate_mean", "hate")
...
plot_care_hate <- plot_data_hate_care_function %>%
  ...
  mutate(line_color = case_when(
    hate[timing == "After"] > hate[timing == "Before"] ~ "grey",  # If After > Before, set color to grey
    hate[timing == "After"] < hate[timing == "Before"] ~ "#A52B4D",  # If After < Before, set color
    TRUE ~ "grey"  # Default case (in case of ties or missing values)
  )) %>%
  ungroup() %>%
  ggplot(aes(x = timing, y = hate, group = response_id, color = line_color)) +  # Use line_color for coloring lines
...
plots_care <- plot_care_bhs + plot_care_shs + plot_care_hate +
  plot_annotation(title = "Individual outcome trajectories from before to after Project CARE",
                  theme = theme(plot.title = element_text(hjust = 0.5)),
                  caption = "Red lines within each plot reflect improvements in hopelessness, agency, self-hate.
...
For those who chose **Project Personality** with data at both time points, we plot individual outcome trajectories.
...
plot_pp_bhs <- plot_data_bhs_pp_function %>%
  ...
  mutate(line_color = case_when(
    hopelessness[timing == "After"] > hopelessness[timing == "Before"] ~ "grey",  # If After > Before, set color to grey
    hopelessness[timing == "After"] < hopelessness[timing == "Before"] ~ "#3F9CF5",  # If After < Before, set color to blue
    TRUE ~ "grey"  # Default case (in case of ties or missing values)
  )) %>%
  ungroup() %>%
  ggplot(aes(x = timing, y = hopelessness, group = response_id, color = line_color)) +  # Use line_color for coloring lines
  geom_line(alpha = .7, size = 1.1) +  # Increase size to make the lines thicker (adjust the value as needed)
  geom_point(size = 2) +  # Add points to highlight individual data points
  scale_color_manual(values = c("#3F9CF5" = "#3F9CF5", "grey" = "grey")) +  # Set color for decreased and increased scores
  theme_light() +
  theme(legend.position = "none",  # Remove legend, as it's not needed for this plot
        text = element_text(family = "noto sans"),
        title = element_text(size = 10),
...
plot_pp_shs <- plot_data_shs_pp_function %>%
  ...
  mutate(line_color = case_when(
    hope[timing == "After"] < hope[timing == "Before"] ~ "grey",  # If After > Before, set color to grey
    hope[timing == "After"] > hope[timing == "Before"] ~ "#3F9CF5",  # If After < Before, set color to blue
    TRUE ~ "grey"  # Default case (in case of ties or missing values)
  )) %>%
  ungroup() %>%
  ggplot(aes(x = timing, y = hope, group = response_id, color = line_color)) +  # Use line_color for coloring lines
...
plot_pp_hate <- plot_data_hate_pp_function %>%
  ...
  mutate(line_color = case_when(
    hate[timing == "After"] > hate[timing == "Before"] ~ "grey",  # If After > Before, set color to grey
    hate[timing == "After"] < hate[timing == "Before"] ~ "#3F9CF5",  # If After < Before, set color to blue
    TRUE ~ "grey"  # Default case (in case of ties or missing values)
  )) %>%
  ungroup() %>%
  ggplot(aes(x = timing, y = hate, group = response_id, color = line_color)) +  # Use line_color for coloring lines
...
plots_pp <- plot_pp_bhs + plot_pp_shs + plot_pp_hate +
   plot_annotation(title = "Individual outcome trajectories from before to after Project Personality",
                  theme = theme(plot.title = element_text(hjust = 0.5)),
                  caption = "Blue lines within each plot reflect improvements in hopelessness, agency, self-hate.
...
```{r combining all pre post outcomes plots, echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=8}
...
plots_all <- plots_abc / plots_care / plots_pp  + 
  plot_annotation(
    title = "Individual outcome trajectories before vs after all SSIs",
    caption = "Hopelessness (left column), agency (middle column), self-hate (right column) before + after each SSI;
    ABC (green/top row), CARE (red/middle row), and Project Personality (blue/bottom row).
    Colored lines represent individual improvements on an outcome over time."
  )
