...
results <- filter_and_summarize_ssi(data, "RISE", "n_started_rise", "n_completed_rise")
print(results)
...
m_pfs_rise <- round(mean(data_rise$pfs_mean, na.rm = TRUE), 2)
...
# BHS
results_bhs <- run_paired_t_test(data_rise, "pre_bhs_mean", "post_bhs_mean", "rise_bhs")
print(results_bhs)

# SHS
results_shs <- run_paired_t_test(data_rise, "pre_shs_mean", "post_shs_mean", "rise_shs")
print(results_shs)

# Self-hate
results_hate <- run_paired_t_test(data_rise, "pre_self_hate_mean", "post_self_hate_mean", "rise_hate")
print(results_hate)
...
# Calculating n youth with no history of formal supports
...
# Calculating perc youth with no history of mh treatment
...
**Executive Summary**: This report uses youth data (under 18) from all English-speaking YES surveys (data_ids: d01, d02, d03, d05, d09, d16). SSIs are completed ~40-50% of the time they are started. Clinical outcomes (e.g., hopelessness, agency, self-hate) improve from before to after finishing an SSI. SSIs receive ratings between 3 and 4 out of 5, where 5 is the highest approval rating. Lots of drop-off happens when SSIs introduce writing exercises.
...
+ `r perc_youth_elev_dep`% report elevated depressive symptoms (MFQ is 12 or higher)
...
+ By far, most responses in this data heard about YES in school
...
+ We have no evidence that adding the "commit to finish this SSI" item has increased completion rates overall
...
Data source(s), of all responses:

```{r prepping for data tally, include=FALSE}

data_data_source <- data %>%
  select(data_id)
...
data_source_tallies <- data_data_source %>%
  group_by(data_id) %>%
  tally() %>%
  ungroup() %>%
  rename(count = n) %>%
  rowwise() %>%
  mutate(percentage = count / nrow(data_data_source) * 100) %>%
  mutate(percentage = round(percentage, 2)) %>%
  unnest(cols = c(percentage)) %>%
  mutate(notes = case_when(data_id == "d01" ~ "original English YES",
                           data_id == "d02" ~ "San Antonio English with Q's",
                           data_id == "d05" ~ "YES Middle East English",
                           data_id == "d09" ~ "current YES English",
                           data_id == "d16" ~ "temporary YES English during move")) %>%
  select(data_id, notes, count, percentage) %>%
  rename(n = count, "%" = percentage)

```

```{r printing data tally, echo=FALSE}
...
**`r perc_gender_differs_from_sex`% of responses endorsed a gender identity different from assigned sex.** Gender(s), of all survey starts from youth (n = `r n_started_survey`): 
...
gender_tallies <- count_yes_responses(data_gender) %>%
...
```{r printing gender tally, echo=FALSE}

rempsyc::nice_table(gender_tallies)

```

```{r perc gender differs from sex, include=FALSE}
...
**`r perc_sex_minority`% of responses endorsed a sexual orientation other than heterosexual.** Sexual orientation(s), of all survey starts from youth (n = `r n_started_survey`): 
...
table(data$dem_sex_orientation)

```

```{r printing sex orientation tally, echo=FALSE}

rempsyc::nice_table(sex_orientation_tallies)

```

```{r printing perc sexual minority, include=FALSE}
...
n_race_denominator <- nrow(data_race)

```

Race/ethnicities, of the number of survey starts, among responses that do not come from d05 (n = `r n_race_denominator`, from d01/d02/d09/d16), given d05 ethnicity is measured differently. Race/ethnicity is not assessed in d03, so these responses are automatically counted as missing.

Races/ethnicities (non-d05):

```{r calculating race tallies, include=FALSE}

## Calculating race tallies

race_tallies <- data_race %>%
  summarise(across(everything(), ~ sum(. == "yes" | . == "Yes", na.rm = TRUE))) %>% # Count "yes" in each column
  pivot_longer(cols = everything(), # Convert to long format
                 names_to = "variable", 
                 values_to = "count") %>%
...
n_ethnicity_denominator <- nrow(data_ethnicity)
...
```{r printing ethnicity tally, echo=FALSE}
...
**Over a quarter of responses (`r perc_youth_elev_dep`%) reported elevated levels of depression symptoms.** Here, we plot depression scale scores (MFQ) across the entire youth sample.
...
data_access_plot <- data %>%
  select(dem_receive_support) %>%
  # filter(!str_detect(dem_receive_support, ",no")) %>% # dropping a single conflicting response + nas
...
plot_access <- ggplot(data_access_plot_mutated, aes(x = dem_receive_support)) +  
  ...
  labs(x = "", y = "Frequency") +
  ylim(0, 1750)  # Extending the y-axis limit
...
Here, we plot the most common words in a qualitative question about where they'd heard about yes. Stop words were removed (e.g., "a", "the", "at") before plotting to allow for interpretation.
...
Here, we plot the most common words in a qualitative question about why they were interested in yes. Stop words were removed (e.g., "a", "the", "at") before plotting to allow for interpretation.
...
+ **hopelessness decreased** from pre to post abc, dz = `r abc_bhs_dz_value`.
...
+ **agency increased** from pre to post abc, dz = `r abc_shs_dz_value`.
...
+ **self-hate decreased** from pre to post abc, dz = `r abc_hate_dz_value`.
...
+ **hopelessness decreased** from pre to post project personality, dz = `r pp_bhs_dz_value`.
...
+ **agency increased** from pre to post project personality, dz = `r pp_shs_dz_value`.
...
+ **self-hate decreased** from pre to post project personality, dz = `r pp_hate_dz_value`.
...
For those who chose **Project RISE** with data at both time points:

+ **No evidence that hopelessness decreased** from pre to post rise, dz = `r rise_bhs_dz_value`.
    + Note: this test is run in a very small sample size (df = `r d.dep.t.diff.t(rise_hate_t_value, rise_hate_n_pairs, a = 0.05)$df`).

```{r t test results rise bhs, echo=FALSE}

# t_rise_bhs # full results here

# Pulling table values

data_t_rise_bhs <- broom::tidy(t_rise_bhs) %>%
  select(statistic, parameter, p.value) %>%
  rename(df = parameter) %>%
  mutate(t = round(statistic, 2)) %>%
  mutate(dz = rise_bhs_dz_value,
         rise_test = "Hopelessness") %>%
  select(rise_test, t, df, p.value, dz)
...
+ **No evidence that agency increased** from pre to post rise, dz = `r rise_shs_dz_value`. 
    + Note: this test is run in a very small sample size (df = `r d.dep.t.diff.t(rise_hate_t_value, rise_hate_n_pairs, a = 0.05)$df`).

```{r t test results rise shs, echo=FALSE}

# t_rise_shs # full results here

# Pulling table values

data_t_rise_shs <- broom::tidy(t_rise_shs) %>%
  select(statistic, parameter, p.value) %>%
  rename(df = parameter) %>%
  mutate(t = round(statistic, 2)) %>%
  mutate(dz = rise_shs_dz_value,
         rise_test = "Agency") %>%
  select(rise_test, t, df, p.value, dz)
...
plot_data_shs_rise_function <- process_plot_data(data, "RISE", "pre_shs_mean", "post_shs_mean", "hope")
...
+ **No evidence that self-hate decreased** from pre to post rise, dz = `r rise_hate_dz_value`. 
    + Note: this test is run in a very small sample size (df = `r d.dep.t.diff.t(rise_hate_t_value, rise_hate_n_pairs, a = 0.05)$df`). 

```{r t test results rise hate, echo=FALSE}

# t_rise_hate # full results here

# Pulling table values

data_t_rise_hate <- broom::tidy(t_rise_hate) %>%
  select(statistic, parameter, p.value) %>%
  rename(df = parameter) %>%
  mutate(t = round(statistic, 2)) %>%
  mutate(dz = rise_hate_dz_value,
         rise_test = "Self-Hate") %>%
  select(rise_test, t, df, p.value, dz)
...
plot_data_hate_rise_function <- process_plot_data(data, "RISE", "pre_self_hate_mean", "post_self_hate_mean", "hate")
...
Paired t-test results for Project RISE:

```{r all rise t results, echo=FALSE}

# Combining t results

t_results_rise <- bind_rows(data_t_rise_bhs, data_t_rise_shs, data_t_rise_hate)

# Now, use nice_table for all abc results

rempsyc::nice_table(t_results_rise)
...
plots_rise <- plot_rise_bhs + plot_rise_shs + plot_rise_hate +
  plot_annotation(title = "Outcomes before vs after Project RISE",
                  theme = theme(plot.title = element_text(hjust = 0.5)),
                  caption = "Larger values within each plot (further right) are greater hopelessness, agency, self-hate.
                  Left plot = hopelessness scores (BHS) before and after RISE;
                  Middle plot = agency scores (SHS) before and after RISE;
                  Right plot = self-hate scores before and after RISE")

plots_rise

```

```{r change rise hopeless, include=FALSE}

# Calculating number a lot or little less hopeless

n_rise_lot_little_less_hopeless <- data_rise %>%
  filter(perc_change_hope == "A lot less hopeless" | perc_change_hope == "A little less hopeless") %>%
  nrow()

# Calculating total responses to that item

n_rise_answered_hopeless <- data_rise %>%
  filter(!is.na(perc_change_hope)) %>%
  nrow()

# Calculating percentage who answered a lot or little less hopeless

perc_rise_lot_little_less_hopeless <- round((n_rise_lot_little_less_hopeless / n_rise_answered_hopeless)*100, 2)

```

Of those who answered this question, **`r perc_rise_lot_little_less_hopeless`% of responses were "a lot" or "a little" less hopeless post-RISE.**

```{r printing rise change hopelessness, include=FALSE}

perc_rise_lot_little_less_hopeless

```

```{r plotting rise change hopelessness, warning=FALSE, message=FALSE, echo=FALSE}

# Creating df

data_rise_plot_change_hope <- data_rise %>%
  filter(!is.na(perc_change_hope)) %>%
  select(perc_change_hope)
...
data_rise_plot_change_hope_mutated <- data_rise_plot_change_hope %>%
  mutate(perc_change_hope = factor(data_rise_plot_change_hope$perc_change_hope, levels = custom_order))

# Plotting 

plot_rise_change_hopeless <- ggplot(data_rise_plot_change_hope_mutated, aes(x = perc_change_hope)) +  
  geom_histogram(stat="count", fill = "#F48151", alpha = 0.8) +  
  theme_light() +
  theme(legend.position = "none",
        title = element_text(size = 10),
        plot.background = element_rect(fill = "white"),
        axis.text.x = element_text(angle = 45, hjust = 1)) +  # rotate x-axis labels
  plot_annotation(title = "Compared to before (Project RISE), how hopeless do you feel right now?") +
  labs(x = "", y = "frequency")

plot_rise_change_hopeless

```

```{r change rise problem solving, include=FALSE}

# Calculating number a lot or little more able to solve problems

n_rise_lot_little_more_prob <- data_rise %>%
  filter(perc_change_prob == "A lot more able to solve the problems" | 
           perc_change_prob == "A little more able to solve the problems") %>%
  nrow()
...
n_rise_answered_prob <- data_rise %>%
  filter(!is.na(perc_change_prob)) %>%
  nrow()

# Calculating percentage who answered a lot or little less hopeless

perc_rise_lot_little_more_prob <- round((n_rise_lot_little_more_prob / n_rise_answered_prob)*100, 2)

```

Of those who answered this question, **`r perc_rise_lot_little_more_prob`% of responses were "a lot" or "a little" more able to solve problems post-RISE.**

```{r printing rise change problem solving, include=FALSE}

perc_rise_lot_little_more_prob

```

```{r plotting rise change problem solving, warning=FALSE, message=FALSE, echo=FALSE}

# Creating df

data_rise_plot_change_prob <- data_rise %>%
  filter(!is.na(perc_change_prob)) %>%
  select(perc_change_prob) %>%
  mutate(perc_change_prob = case_when(perc_change_prob == "A lot more able to solve the problems" ~ "A lot more able",
                                      perc_change_prob == "A little more able to solve the problems" ~ "A little more able",
...
data_rise_plot_change_prob_mutated <- data_rise_plot_change_prob %>%
  mutate(perc_change_prob = factor(data_rise_plot_change_prob$perc_change_prob, levels = custom_order))

# Plotting 

plot_rise_change_prob <- ggplot(data_rise_plot_change_prob_mutated, aes(x = perc_change_prob)) +  
  geom_histogram(stat="count", fill = "#F48151", alpha = 0.8) +  
  theme_light() +
  theme(legend.position = "none",
        title = element_text(size = 10),
        plot.background = element_rect(fill = "white"),
        axis.text.x = element_text(angle = 45, hjust = 1)) +  # rotate x-axis labels
  plot_annotation(title = "Compared to before (Project RISE), how able are you to solve your problems?") +
  labs(x = "", y = "frequency")

plot_rise_change_prob
...
plots_all <- plots_abc / plots_care / plots_pp / plots_rise  + 
  plot_annotation(
    ...
    caption = "hopelessness, agency, self-hate before + after ABC (green), CARE (red), Project Personality (blue), and RISE (orange)"
  )
...
```{r rise completion rate calc, include=FALSE}

# Calculating completion rates rise

completions_rise <- data_rise %>%
  summarise(
    ssi_starts = sum(!is.na(ssi_started)),
    ssi_completions = sum(!is.na(ssi_completed)),
    completion_rate = round((ssi_completions / ssi_starts) * 100, 2)) %>%
  mutate(ssi_group = "Project RISE") %>%
  select(ssi_group, everything())
...
Within d02, we have SSI completion rate data before and after introducing commitment items (where teens "commit" to finishing the SSI they chose). Here, we plot SSI completions by date, indicating when the commit items were introduced.
...
data_commit_by_date <- data %>%
  select(start_date, abc_commit, proj_care_commit, proj_pers_commit, proj_rise_commit, data_id)

# Pulling earliest start_date with commit data within each data_id
  
date_commit_introduced_by_data_id <- data_commit_by_date %>%
  filter(!is.na(abc_commit) | !is.na(proj_care_commit) | !is.na(proj_pers_commit) | !is.na(proj_rise_commit)) %>%
  arrange(start_date) %>%
  group_by(data_id) %>%
  slice(1)
...
```{r comparing first observation by data id vs first commit data by data id, include=FALSE}

data_commit_date_d01 <- data_commit_date_bound %>%
  filter(data_id == "d01")  # no commit data 

data_commit_date_d02 <- data_commit_date_bound %>%
  filter(data_id == "d02")  # first observation vs first commit data

data_commit_date_d05 <- data_commit_date_bound %>%
  filter(data_id == "d05")  # no commit data 

data_commit_date_d09 <- data_commit_date_bound %>%
  filter(data_id == "d09")  # no pre-commit data 

data_commit_date_d16 <- data_commit_date_bound %>%
  filter(data_id == "d16")  # no pre-commit data 

```

```{r pulling data points for commit and completions by date plot, include=FALSE}

# d02 is the only data_id with pre and post commitment data
## (first observation is much earlier than first commit data, 
## was originally na for all commit vars)

data_completion_by_date_d02 <- data %>%
  select(start_date, abc_commit, proj_care_commit, proj_pers_commit, 
         proj_rise_commit, ssi_started, ssi_completed, data_id) %>%
  filter(data_id == "d02") %>%
  arrange(start_date)

# Pulling first start_date where commitment qs were introduced 

date_d02_commit_introduced <- data_completion_by_date_d02 %>%
  filter(!is.na(abc_commit) | !is.na(proj_care_commit) | !is.na(proj_pers_commit) | !is.na(proj_rise_commit)) %>%
  arrange(start_date) %>%
  slice(1) %>%
  select(start_date) %>%
...
plot_data_completions_per_date <- data_completion_by_date_d02 %>%
  group_by(start_date) %>%
  filter(!is.na(ssi_completed)) %>%
  tally() %>%
  ungroup()
...
It's clear our San Antonio partners had many students complete an SSI on one day in late Dec of 2021. 
...
plot_completions_by_date <- ggplot(plot_data_completions_per_date, aes(x = start_date, y = n)) +   
  ...
  theme_minimal() +
  labs(x = "", y = "n ssis completed", title = "Daily SSI Completions (d02 only)") +
  theme(legend.position = "none") +
  geom_vline(xintercept = date_d02_commit_introduced, color = "black", linetype = "dotted") +
  plot_annotation(caption = paste("Taller bars indicate more completions on a specific date. 
                  Dotted black line shows the date we introduced 'commit' items to SSIs:", date_d02_commit_introduced,"."))

plot_completions_by_date

```

Here we calculate SSI completion rates before vs after introducing the commit items to d02 (date we added commit item: `r date_d02_commit_introduced`). Given the large number of SSI completions in mid-Dec, we also compare SSI completion rates pre-commitment item to those after the unusual spike in completions. When excluding completions from the SA event period, completion rates look very similar in these data from pre to post adding the commitment item.

We likely get a large completion rate boost due to people in SA encouraging youth to complete an SSI during this mid-Dec window.

```{r completion rate data pre commit item, include=FALSE}

# Calculating completion rate data pre and post commit item

completion_rate_pre_commit <- data_completion_by_date_d02 %>%
  filter(start_date < date_d02_commit_introduced) %>%
  summarise(
    ssi_starts = sum(!is.na(ssi_started)),
    ssi_completions = sum(!is.na(ssi_completed)),
    completion_rate = round((ssi_completions / ssi_starts) * 100, 2)) %>%
  mutate(timing = "before commit item and SA event") %>%
  select(timing, everything())

completion_rate_post_commit <- data_completion_by_date_d02 %>%
  filter(start_date > date_d02_commit_introduced) %>%
  summarise(
    ssi_starts = sum(!is.na(ssi_started)),
    ssi_completions = sum(!is.na(ssi_completed)),
    completion_rate = round((ssi_completions / ssi_starts) * 100, 2)) %>%
  mutate(timing = "after commit item (including SA event)") %>%
  select(timing, everything())

completion_rate_post_sa_push <- data_completion_by_date_d02 %>%
  filter(start_date > "2021-12-17") %>%
  summarise(
    ssi_starts = sum(!is.na(ssi_started)),
    ssi_completions = sum(!is.na(ssi_completed)),
    completion_rate = round((ssi_completions / ssi_starts) * 100, 2)) %>%
  mutate(timing = "after commit item (excluding SA event)") %>%
  select(timing, everything())

# Binding rows

completion_rates_pre_and_post_commit <- bind_rows(completion_rate_pre_commit, 
                                                  completion_rate_post_commit,
                                                  completion_rate_post_sa_push)

```

```{r printing completion rate data pre commit item, echo=FALSE}

rempsyc::nice_table(completion_rates_pre_and_post_commit)
...
```{r pfs item desctiptives for rise, echo=FALSE}

## Running custom function to get pfs item descriptives 

summary_pfs_rise <- calculate_pfs_item_means(data_rise)
...
data_rise_ridge_plot <- data_rise %>%
  select(starts_with("pfs_") & 
         -contains("like") & -contains("change"), 
         -contains("mean"), -contains("feedback"), 
         -contains("tim")) %>%
...
Given extensive display logic, we can't confidently calculate largest page for drop-off in ABC.

```{r selecting abc pages for plot, include=FALSE}

## Selecting only abc pages

data_abc_clicks <- data_abc %>%
...
Counting number of page submissions by page number (looking for drop-off). Pulling largest drop-off point during this SSI.

```{r selecting care pages for plot, include=FALSE}

## Selecting only care pages

data_care_clicks <- data_care %>%
...
```{r care page plot, include=FALSE}
...
Plotting drop-off for all SSIs. Curve slopes are steeper where drop-off is greater.
...
plot_all_drop <- plot_abc_drop + plot_care_drop + plot_pp_drop + 
  plot_annotation(
    ...
    caption = "ABC (green/left), CARE (red/middle), Project Personality (blue/right); 
    Vertical lines are pages with writing exercises."
  )
