...
```{r, out.width = "350px", out.height = "120px", include=TRUE, echo=FALSE}
...
**Here we share results from >5,500 youth across the globe who have tried our open-access SSIs** (data collection ongoing). From before to after trying an SSI online, youth report improvements on key mental health outcomes (hopelessness, agency, self-hate).
...
Project YES helped teens feel less alone, kinder toward themselves, and capable to creating a personal action plan. Here are real testimonials from youth who visited our platform:
...
Our Project YES resource has already reached a wide range of youth, across age, gender, and racial/ethnic identity.

## Age

Ages of all YES responses from youth (n = `r n_started_survey`). 
...
Genders of all YES responses from youth (n = `r n_started_survey`): 
...
Race/Ethnicity of all YES responses from youth, where race and ethnicity were collected according to U.S. guidelines (n = `r n_race_denominator`). 
...
**Over a quarter of responses (`r perc_youth_elev_dep`%) reported elevated levels of depression symptoms.** Here, we plot depression scale scores, using the [Mood and Feelings Questionnaire (MFQ)](https://doi.org/10.1002/mpr.1610), across the entire youth sample.

This means *more than 1 in 4 youth who accessed our Project YES platform* were possibly experiencing clinical levels of things like loneliness, self-hate, low motivation, low concentration, and low mood.
