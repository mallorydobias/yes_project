---
title: "yes_english_data_binding"
...
english_data_bound <- bind_rows(d02_data, d03_data, d05_data, d09_data, d16_data)
...
validation_result <- 
  english_data_bound %>%
  expect_row_count_match(nrow(d02_data) + nrow(d03_data) + nrow(d05_data) + nrow(d09_data) + + nrow(d16_data))
...
data_cols <- data.frame(column_name = names(english_data_bound))
...
```{r checking n columns d02, validate = TRUE}
...
validation_result <- 
  n_expected_cols_d02 %>%
  expect_row_count_match(nrow(n_actual_cols_d02))
...
validation_result_d02 <- validate_column_counts(d02_cols, data_cols)
...
validation_result_d03 <- validate_column_counts(d03_cols, data_cols)
...
validation_result_d05 <- validate_column_counts(d05_cols, data_cols)
...
validation_result_d09 <- validate_column_counts(d09_cols, data_cols)
...
validation_result_d16 <- validate_column_counts(d16_cols, data_cols)
...
english_data_reorged <- english_data_bound %>%
  select(response_id, language, data_id, contains("date"),
         ...
         contains("hear_proj"), dem_why_interest, dem_receive_support, dem_why_no_support, dem_adult, dem_age, dem_sex,
