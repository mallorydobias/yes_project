...
Changing "pi_" (post-intervention) variable prefix to "post_" prefix, for clarity
...
data_mfq_mutated %>%
  pointblank::col_is_numeric(vars(mfq_1)) # passes if mfq vars are now numeric
...
```{r bhs numeric class validation, validate=TRUE}

data_bhs_mutated %>%
  pointblank::col_is_numeric(vars(pre_bhs_1)) # passes if vars are now numeric
...
Calculating bhs composite scores
...
data_bhs_scored <- data_bhs_mutated %>%
  mutate(pre_bhs_total = rowSums(across(starts_with("pre_bhs_") & -contains("tim"))),
         post_bhs_total = rowSums(across(starts_with("post_bhs_") & -contains("tim")))) 
...
test_pre_bhs_calc <- data_bhs_scored %>%
  ...
  mutate(pre_manual_total = pre_bhs_1 + pre_bhs_2 + pre_bhs_3 + pre_bhs_4) %>%
  select(pre_bhs_total, pre_manual_total)
...
test_post_bhs_calc <- data_bhs_scored %>%
  ...
  mutate(post_manual_total = post_bhs_1 + post_bhs_2 + post_bhs_3 + post_bhs_4) %>%
  select(post_bhs_total, post_manual_total)

```

Checking pre and post bhs totals are identical across sum methods

```{r pre bhs scoring validation, validate = TRUE}

test_pre_bhs_calc %>%
  pointblank::col_vals_equal(vars(pre_bhs_total), 
                             vars(pre_manual_total), 
...
test_post_bhs_calc %>%
  pointblank::col_vals_equal(vars(post_bhs_total), 
                             vars(post_manual_total), 
                             na_pass = TRUE) # passes if totals are identical across sum methods
