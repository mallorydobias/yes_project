...
```{r checking rise var naming, validate = TRUE}
...
# running pointblank code

test_rise_naming_df %>%
  pointblank::test_col_vals_equal(vars(test_rise_pre), 
                                  vars(test_rise_post), 
                                  threshold = 1) # true if renamed columns (except text cols) are identical to original cols
...
```{r test chunk, validate = TRUE}

data_mfq_mutated %>%
  pointblank::col_is_character(vars(mfq_1)) 
