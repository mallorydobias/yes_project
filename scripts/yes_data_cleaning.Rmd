...
Renaming perceived control items for clarity

```{r renaming perceived control vars}

## Renaming control vars for clarity

data_control_named <- data_post_named %>%
  mutate(pre_perceived_control = pre_control_1, 
...
```{r checking control var renaming, validate = TRUE}

test_rename <- data_control_named

test_rename %>%
  pointblank::col_vals_equal(vars(pre_perceived_control), 
                             vars(pre_control_1)) %>%
  pointblank::col_vals_equal(vars(post_perceived_control), 
                             vars(post_control_1)) # passes if renamed pre column is identical to original column

```

Renaming status items items for clarity and consistency

```{r renaming status vars}

## Renaming status vars for clarity

data_status_named <- data_control_named %>%
  mutate(perc_progress = progress, 
...
```{r checking status var renaming, validate = TRUE}

test_rename <- data_status_named

test_rename %>%
  pointblank::col_vals_equal(vars(perc_progress), 
                             vars(progress)) %>%
  pointblank::col_vals_equal(vars(volunteer_questionnaires_yn), 
                             vars(volunteer)) %>%
  pointblank::col_vals_equal(vars(ssi_started), 
                             vars(project_started)) %>%
  pointblank::col_vals_equal(vars(ssi_completed), 
                             vars(project_completed)) # passes if renamed pre column is identical to original column
...
data_mfq_mutated <- data_status_named %>%
...
Dropping unnecessary columns and columns that are scale scores calculated within qualtrics
...
data_col_dropped <- data_pfs_scored %>%
  select(-status, -contains("duration"), -contains("recipient"), -contains("external"), -contains("distribution"), 
         -recorded_date, -user_language, -contains("control_1"), -progress, -volunteer, -questionnaires, 
         -project_started, -project_completed, -sc0, -sc1, -sc2) # dropping unnecessary qualtrics metadata and scale scores
...
data_reorged <- data_col_added %>%
  select(response_id, language, data_id, contains("date"), ip_address, contains("location"), 
         perc_progress, finished, ssi_source, ssi_choose, 
         contains("volunteer") & !contains("tim"),
         contains("dem"), starts_with("prog"), contains("tim"))
