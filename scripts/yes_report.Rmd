...
## Calculating percentage of these cases, of everyone who agreed to questionnaires

perc_gender_differs_from_sex <- round((n_gender_differs_from_sex / n_agreed_questionnaires)*100,2)
...
Across all survey cases:
+ `r perc_gender_differs_from_sex`% endorse a gender identity that differs from sex assigned at birth
...
N's and percentages are all calculated out of the number of responses where youth agreed to complete questionnaires (n = `r n_agreed_questionnaires`).
...
age_tallies <- data_youth %>%
  ...
  rename(count = n) %>%
  rowwise() %>%
  mutate(percentage = count / n_agreed_questionnaires * 100) %>%
...
Of responses where youth agreed to complete questionnaires, `r perc_gender_differs_from_sex`% of survey responses endorsed a gender identity that differs from sex assigned at birth. N's and percentages are all calculated out of the number of responses where youth agreed to complete questionnaires (n = `r n_agreed_questionnaires`). 
...
data_youth_gender <- data_youth %>%
  ...
  filter(volunteer_questionnaires_yn == "Yes") %>% # filtering to those who agreed to questionnaires
