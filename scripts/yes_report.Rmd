...
# Combine all dz values into a single vector for reporting
care_dz_values <- c(care_bhs_dz_value) # not including non-significant outcome results
...
# Combine all dz values into a single vector for reporting
pp_dz_values <- c(pp_shs_dz_value) # not including non-significant outcome results
print(pp_dz_values)
...
**Executive Summary**: This report uses youth data (under 18) from the largest English-speaking survey among YES data sources (data_id: d09). SSIs are completed ~1 out of every 3 times they are started. Clinical outcomes (e.g., hopelessness, agency, self-hate) improve from before to after finishing an SSI, but not for every outcome for all SSIs. SSIs receive ratings between 3 and 4 out of 5, where 5 is the highest approval rating. Lots of drop-off happens when SSIs introduce writing exercises.
...
+ `r perc_racial_minority`% identify as Asian, Black, or another racial identity not listed (`r perc_racial_white`% white)
...
+ Hopelessness decreased pre to post CARE (dzs = `r care_dz_values`)
+ Agency increased pre to post Project Personality (dzs = `r pp_dz_values`)
...
+ Nearly 1 in 5 people drop from one page to the next during Project Personality's first writing exercise
...
`r perc_racial_minority`% of survey responses identified as Asian, Black, or another racial identity not listed. `r perc_racial_latine_hisp`% identify as Latine or Hispanic. N's and percentages are all calculated out of the number of survey starts (n = `r n_started_survey`). 
...
Conducting a paired t-test to evaluate pre vs post-care changes in self-hate (self-hate scale scores). For those who chose the care intervention with data at both time points, we detect no significant change in self-hate from pre to post care, dz = `r abc_hate_dz_value`.
...
Conducting a paired t-test to evaluate pre vs post-project personality changes in hopelessness (bhs scores). For those who chose the project personality intervention with data at both time points, we detect no significant change in hopelessness from pre to post project personality, dz = `r pp_bhs_dz_value`.
...
Conducting a paired t-test to evaluate pre vs post-project personality changes in self-hate (self-hate scale scores). For those who chose the project personality intervention with data at both time points, we detect no significant change in self-hate from pre to post project personality, dz = `r pp_hate_dz_value`.
