...
Within d02, we have SSI completion rate data before and after introducing commitment items (where teens "commit" to finishing the SSI they chose). Here, we plot SSI completions by date, as well as SSI completion rates by date. We indicate when the commit items were introduced.
...
Overall, here we calculate SSI completion rates before vs after introducing the commit items to this data source (date we added commit item: `r date_d02_commit_introduced`).
...
completion_rate_pre_commit <- data_completion_by_date_d02 %>%
  ...
  mutate(timing = "before commit item") %>%
...
completion_rates_pre_and_post_commit <- bind_rows(completion_rate_pre_commit, completion_rate_post_commit)
