...
This report uses youth data (under 18) from the largest English-speaking survey among YES data sources. The report includes examples of outcomes/results Mallory can generate for anyone interested in these data. For example, in these data to date:
...
m_pfs_enjoyed <- round(mean(data_youth$pfs_1, na.rm = TRUE), 2)

m_pfs_understood <- round(mean(data_youth$pfs_2, na.rm = TRUE), 2)

m_pfs_easy <- round(mean(data_youth$pfs_3, na.rm = TRUE), 2)

m_pfs_tried <- round(mean(data_youth$pfs_4, na.rm = TRUE), 2)

m_pfs_helpful <- round(mean(data_youth$pfs_5, na.rm = TRUE), 2)

m_pfs_recommend <- round(mean(data_youth$pfs_6, na.rm = TRUE), 2)

m_pfs_agree <- round(mean(data_youth$pfs_7, na.rm = TRUE), 2)
