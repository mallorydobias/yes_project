...
perc_youth_elev_dep <- round((n_youth_elev_dep / nrow(data_youth))*100, 2)

```

Over a quarter of cases (`r perc_youth_elev_dep`%) reported elevated levels of depression symptoms. Here, we plot depression scale scores (MFQ) across the entire youth sample.

```{r plotting mfq, warning=FALSE, message=FALSE}
...
plot_mfq <- ggplot(data_mfq_plot, aes(x = mfq_total, fill = mfq_category)) +  # conditional fill
  ...
  plot_annotation(title = "Depression scale scores (MFQ) for all youth",
                  ...
                  Scores of 12 or higher suggest possible depression (darker purple). 
                  Mean score =", round(mfq_mean$mfq_mean, 2), "(solid line)."))
