...
data_completion_by_date_plot <- data_youth %>%
  select(start_date, volunteer_questionnaires_yn, data_id)

# Pulling earliest start_date with optional qs data within each data_id
  
date_optional_qs_introduced <- data_completion_by_date_plot %>%
  filter(!is.na(volunteer_questionnaires_yn)) %>%
  arrange(start_date) %>%
  group_by(data_id) %>%
  slice(1)

date_first_observation <- data_completion_by_date_plot %>%
  arrange(start_date) %>%
  group_by(data_id) %>%
  slice(1) # d09 is the only data_id with pre and post commitment data

# Pulling first start_date where optional qs were introduced in d09

date_d09_optional_qs_introduced <- data_completion_by_date_plot %>%
  filter(!is.na(volunteer_questionnaires_yn) & data_id == "d09") %>%
  arrange(start_date) %>%
  slice(1) %>%
  select(start_date) %>%
  pull()
...
```{r plotting d09 completions by start_date}
