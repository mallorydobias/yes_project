...
More than half of the time (`r perc_agreed_questionnaires` %), youth agreed to fill out optional questionnaires.
...
race_tallies <- count_yes_responses(data_youth_race) %>%
  ...
  rename(racial_identity = variable) %>%
  rename("%" = n) %>%
  rename(n = count) %>%
  arrange(-n) %>%
  mutate(racial_identity = case_when(racial_identity == "missing" ~ "Missing",
                                     racial_identity == "white" ~ "White",
                                     racial_identity == "latin_hisp" ~ "Latine/Hispanic",
                                     racial_identity == "asian" ~ "Asian",
                                     racial_identity == "black" ~ "Black",
                                     racial_identity == "other" ~ "Another racial/ethnic identity not listed",
