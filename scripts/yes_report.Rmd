...
t_self_hate <- t.test(data_youth_abc$post_bhs_mean, data_youth_abc$pre_bhs_mean, paired = TRUE)

## Saving p-value for report

abc_bhs_p_value <- t_self_hate$p.value

## Saving t-value for effect size calc

abc_bhs_t_value <- t_self_hate$statistic
...
This report uses youth data (under 18) from the largest English-speaking survey among YES data sources (data_id: d09). In these data to date, our YES survey was started `r n_started_survey` times. `r perc_chose_any_ssi`% of cases chose an SSI after starting (n = `r n_chose_any_ssi`). Of the cases where someone chose an SSI:
...
Conducting a paired t-test to evaluate pre vs post-abc changes in hopelessness (bhs scores). For those who chose the abc intervention with data at both time points, hopelessness decreased from pre to post abc, p = `r abc_bhs_p_value`, dz = `r abc_bhs_dz_value`.
