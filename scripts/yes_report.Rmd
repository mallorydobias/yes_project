...
data_completion_by_date_plot <- data_youth %>%
...
date_commit_introduced_by_data_id <- data_completion_by_date_plot %>%
  filter(!is.na(abc_commit) | !is.na(proj_care_commit) | !is.na(proj_pers_commit) | !is.na(proj_rise_commit)) %>%
  arrange(start_date) %>%
  group_by(data_id) %>%
  slice(1)

date_first_observation_per_data_id <- data_completion_by_date_plot %>%
  arrange(start_date) %>%
  group_by(data_id) %>%
  slice(1) 
...
data_completion_by_date_plot %>%
  filter(data_id == "d02") %>%
  arrange(start_date)

# Pulling first start_date where commitment qs were introduced 

date_d02_commit_introduced <- data_completion_by_date_plot %>%
  filter(data_id == "d02") %>%
  filter(!is.na(abc_commit) | !is.na(proj_care_commit) | !is.na(proj_pers_commit) | !is.na(proj_rise_commit)) %>%
  arrange(start_date) %>%
  slice(1) %>%
...
```{r plotting d09 completions by start_date, include=FALSE}

## MD this is where I'll plot completions by start_date in d09
## Showing the exact point where the optional 
