...
data_youth_gender <- data_youth %>%
  ...
  mutate(dem_gender_missing = case_when(
    if_all(everything(), is.na) ~ "yes",  # "yes" if all gender vars are NA
...
count_yes_responses <- function(data) {
  ...
  data %>%
    ...
    pivot_longer(cols = everything(), # Convert to long format
                 ...
                 values_to = "count") %>%
    rowwise() %>%
    mutate(percentage = round((count / n_agreed_questionnaires) * 100, 2)) %>% # Correct rounding
