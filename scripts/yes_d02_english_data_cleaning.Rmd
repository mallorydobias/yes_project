...
raw_data <- read.csv(here("data", "raw", "Project YES - ENGLISH AND SPANISH_February 13, 2025_13.28.csv")) # directing to data from project folder root
...
data_items_named <- data_intro_named %>%
  rename(san_antonio_hear_proj = san_antonio_how,
         san_antonio_hear_proj_text = san_antonio_how_9_text,
         dem_sex_orientation_other_text = dem_sex_orientation_9_text,
         ...
         ssi_prev_completed_name = ssi_prev_activity,
...
data_race_cleaned <- data_dated %>%
  mutate(dem_race_asian = case_when(
    str_detect(dem_ethnicity, "Asian") ~ "yes",
    !str_detect(dem_ethnicity, "Asian") & !is.na(dem_ethnicity) ~ "no",
    TRUE ~ NA_character_)) %>%
  mutate(dem_race_black = case_when(
    str_detect(dem_ethnicity, "African American") ~ "yes",
    !str_detect(dem_ethnicity, "African American") & !is.na(dem_ethnicity) ~ "no",
    TRUE ~ NA_character_)) %>%
  mutate(dem_race_latin_hisp = case_when(
    str_detect(dem_ethnicity, "Latino/Hispanic") ~ "yes",
    !str_detect(dem_ethnicity, "Latino/Hispanic") & !is.na(dem_ethnicity) ~ "no",
    TRUE ~ NA_character_)) %>%
  mutate(dem_race_white = case_when(
    str_detect(dem_ethnicity, "Caucasian") ~ "yes",
    !str_detect(dem_ethnicity, "Caucasian") & !is.na(dem_ethnicity) ~ "no",
    TRUE ~ NA_character_)) %>%
  mutate(dem_race_other = case_when(
    str_detect(dem_ethnicity, "Other") ~ "yes",
    !str_detect(dem_ethnicity, "Other") & !is.na(dem_ethnicity) ~ "no",
    TRUE ~ NA_character_))
...
test_pre_mutuate_count <- data_dated %>%
  filter(str_detect(dem_ethnicity, "Asian")) %>%
...
test_pre_mutuate_count <- data_gender_other_cleaned %>%
...
## Dropping Testing Rows

```{r dropping testing rows}

## Filtering to rows that are na for dem_hear_proj, or that contain text that don't specify
## that it's a testing row

data_testing_rows_dropped <- data_english_vars_only %>% 
  filter(is.na(dem_hear_proj) | !str_detect(dem_hear_proj, "testing - UTTH") & !str_detect(dem_hear_proj, "testing-UTTH"))

```

Checking data has expected number of rows

```{r checking n rows after dropping testing rows, validate = TRUE}

## Calculating number of testing rows

n_testing_rows <- data_english_vars_only %>% 
  filter(str_detect(dem_hear_proj, "testing - UTTH") | str_detect(dem_hear_proj, "testing-UTTH")) %>%
  select(dem_hear_proj) %>%
  nrow()

## Checking if the row count of data equals expected count

validation_result <- 
  data_testing_rows_dropped %>%
  expect_row_count_match(nrow(data_english_vars_only) - n_testing_rows)
...
data_mfq_mutated <- data_testing_rows_dropped %>%
  mutate(across(starts_with("mfq_") & -contains("tim") & -starts_with("e_"), 
                ~ case_when(
                    . == "Not True" ~ 0,
                    . == "Sometimes" ~ 1,
                    . == "True" ~ 2,
...
data_mfq_mutated %>%
  select(contains("mfq") & -contains("tim") & -starts_with("e_")) %>%
...
test_mfq_pre <- data_testing_rows_dropped %>%
...
test_mutate <- prep_mutation_check(data_testing_rows_dropped, data_mfq_mutated, "mfq_1") # same output as test_mfq_mutate above
...
data_mfq_scored <- data_mfq_mutated %>%
  mutate(mfq_total = case_when(
      if_all(starts_with("mfq_") & !contains("tim"), is.na) ~ NA_real_, # NA_real_ for numeric NA
      TRUE ~ rowSums(across(starts_with("mfq_") & !contains("tim")), na.rm = TRUE))) %>%
  select(-e_mfq_total) # replacing the var calculated within qualtrics
...
Checking pre and post hate means are identical across averaging methods; NAs for these rows because only later rows have values
...
data_col_dropped <- data_pfs_scored %>%
  ...
  select(-dem_ethnicity, -dem_gender_identity) %>% # dropping outdated cols cleaned above
...
data_classes_mutated <- data_reorged %>%
  ...
  mutate(language = as.factor(language),
         ...
         reliant_mg = as.factor(reliant_mg),
  ...
  mutate(across(matches("^san_antonio.*") & !matches("text"), as.factor)) %>%
...
file_name <- "yes_data_d02_english_clean.Rds"
